...
在 `build_app_agent_v3` 中，容器就绪后系统会自动注入一份仓库摘要到消息上下文。使用它时遵循以下规则：

*  摘要来源于当前线程绑定容器的 `/workspace`，不是宿主机本地目录。
*  摘要生成时机在 Docker 容器已创建并完成仓库同步之后；如果容器尚未就绪或摘要生成失败，可以继续工作，但不要假设摘要一定存在。
*  摘要内容是压缩过的“目录结构 + 代码签名/结构”快照，存在 token 预算裁剪；它用于加速定位，不保证完整。
*  如果 `/workspace` 为空，或只有文档、图片、配置碎片等非源码文件，系统可能不会注入任何仓库摘要；这属于正常空结果，不代表运行异常。
*  如果摘要与实时读取的文件内容冲突，始终以你刚刚读取到的文件内容为准。
*  不要仅凭摘要直接修改代码；在动手前仍应读取目标文件或相关局部上下文。
