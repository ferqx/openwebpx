...
_SHELL_WRITE_REDIRECTION_RE = re.compile(
    r"(?:^|[^\S\r\n])(?:cat|echo|printf)\b[^\n]*?(?:>|>>)\s*([^\s<]+)"
)
...
class SandboxPolicyGuard:
    ...
    def _validate_file_op_segment(self, segment: str) -> str | None:
        ...
        if command_name == "tee":
            return self._validate_shell_write_command(segment, tokens)
        if command_name in {"cat", "echo", "printf"}:
            return self._validate_shell_write_command(segment, tokens)
        return None

    def _validate_shell_write_command(
        self,
        segment: str,
        tokens: list[str],
    ) -> str | None:
        ...
        if command_name == "tee":
            ...
            return self._policy_violation(
                reason="shell-based file writes via `tee` are blocked.",
                safer_alternative=(
                    "use the registered `apply_patch` tool for Add/Update/Delete File "
                    "operations instead of writing files through `execute`."
                ),
            )

        match = _SHELL_WRITE_REDIRECTION_RE.search(segment)
        if match is None:
            return None
        target = match.group(1).strip()
        if not target:
            return None
        path_violation = self._validate_paths_in_workspace([target])
        if path_violation is not None:
            return path_violation
        return self._policy_violation(
            reason="shell redirection for file writes is blocked.",
            safer_alternative=(
                "use the registered `apply_patch` tool for Add/Update/Delete File "
                "operations instead of `cat > file`, heredoc, or `echo > file` via `execute`."
            ),
        )
